...
-}
    editSolutionInfo f = checkThenDo
        (to $ const Nothing)
        (Solution.solutionInfo %~ f)
    addProject pji = checkThenDo 
        (Solution.uncheckProject pji)
        (at pji ?~ Project.new pji)
    removeProject pji = checkThenDo
        (Solution.checkProject pji)
        (sans pji)
    getProjects = checkThenGet
        (to $ const Nothing)
        Solution.getProjects
    editProjectInfo pji f = checkThenDo
        (Solution.checkProject pji)
        (ix pji . Project.projectInfo %~ f)
